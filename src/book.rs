...
impl LimitOrderBook {
    ...
    pub fn top_n_sum(&self, side: Side, depth: usize) -> Quantity {
        ...
        match side {
            Side::Bid => self.bids.values().rev().take(depth).sum(),
            Side::Ask => self.asks.values().take(depth).sum(),
        }
    }
    ...
    fn nth_price(&self, side: Side, depth: usize) -> Option<Price> {
        ...
        match side {
            Side::Bid => self.bids.keys().rev().nth(depth - 1).copied(),
            Side::Ask => self.asks.keys().nth(depth - 1).copied(),
        }
    }
    ...
    pub fn update_level(&mut self, side: Side, price: Price, new_qty: Quantity) -> LevelChange {
        let old_best = match side {
            Side::Bid => self.best_bid().map(|(p, _)| p),
        ...
        let old_qty = map.get(&price).copied().unwrap_or(0);
